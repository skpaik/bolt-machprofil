 ...
 */
function handleListJSON(data: any[], outputDir: string) {
  ensureDir(outputDir);
  ...
  data.forEach((item) => {
    ...
    const md = matter.stringify(body || content || "", { id, ...frontmatter });

    fs.writeFileSync(path.join(outputDir, `${id}.md`), md);
  });
}
...
function handleSingleObjectJSON(
  ...
  fs.writeFileSync(path.join(outputDir, `${fileName}.md`), md);
}
