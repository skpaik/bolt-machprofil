...
export default function CertificatesPage() {
    ...
    const filteredCertificates = useMemo(() => {
        let filtered = certificates.filter(cert => {
            ...
            if (selectedStatus !== 'all') {
                const isExpired = cert.expiryDate !== 'No Expiry' &&
    ...
    const featuredCertificates = currentCertificates.filter(cert => cert.featured);
    const regularCertificates = currentCertificates.filter(cert => !cert.featured);
    ...
    const CertificateCard = ({ cert }: { cert: Certificate }) => {
        ...
        return (
            <Card className={`hover:shadow-lg transition-shadow ${cert.featured ? 'border-2 border-primary' : ''}`}>
                {cert.featured && (
