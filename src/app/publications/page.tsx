...
export default function PublicationsPage() {
    const { appConfig,  publicationContentData, langI18n, contentData } = usePortfolio();
    ...
    const types = useMemo(() => {
        const uniqueTypes = [...new Set(publications.map((p: Publication) => p.type))];
        return uniqueTypes.sort();
    }, [publications]);

    const statuses = useMemo(() => {
        const uniqueStatuses = [...new Set(publications.map((p: Publication) => p.status).filter(Boolean))];
    ...
    const featuredPublications = currentPublications.filter((p: Publication) => p.featured);
    const regularPublications = currentPublications.filter((p: Publication) => !p.featured);
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            ...
            case 'Book Chapter': return 'outline';
            case 'Workshop': return 'secondary';
            case 'Preprint': return 'outline';
            case 'Thesis': return 'default';
            case 'Patent': return 'destructive';
    ...
    const getStatusColor = (status?: string) => {
        switch (status) {
            ...
            case 'In Press': return 'secondary';
    ...
    const PublicationCard = ({ pub }: { pub: Publication }) => (
        <Card className={`hover:shadow-lg transition-shadow ${pub.featured ? 'border-2 border-primary' : ''}`}>
            {pub.featured && (
            ...
            <CardHeader>
                <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex gap-2">
                        ...
                        {pub.status && (
                            <Badge variant={getStatusColor(pub.status) as any}>{pub.status}</Badge>
                        )}
                    </div>
                    {pub.citations && (
                ...
                <div className="space-y-2 text-sm">
                    <div className="flex items-start gap-2">
                        {showLucidIcon('users', 'w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0')}
              ...
              {pub.authors.join(', ')}
                    ...
                    <div className="flex items-start gap-2">
                        {showLucidIcon('book-open', 'w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0')}
                        <span className="text-muted-foreground">
              {pub.publishedIn}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <Card className="text-center p-4">
                    ...
                    <div className="text-2xl font-bold">
                        {publications.reduce((sum: number, p: Publication) => sum + (p.citations || 0), 0)}
                    </div>
